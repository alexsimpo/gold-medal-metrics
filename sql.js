...
/*
Returns a SQL query string that will find the year where the given country 
won the most wintEGERer medals, along with the number of medals aliased to 'count'.
*/
...
const bestYear = country => {
  return `SELECT COUNT(*) AS count, year 
    FROM GoldMedal
    WHERE country = '${country}'
    GROUP BY year
    ORDER BY count DESC
...
const bestDiscipline = country => {
  return `SELECT COUNT(*) AS count, discipline 
  FROM GoldMedal
  WHERE country = '${country}'
  GROUP BY discipline
  ORDER BY count DESC
...
const bestSport = country => {
  return `SELECT COUNT(*) AS count, sport 
  FROM GoldMedal
  WHERE country = '${country}'
  GROUP BY sport
  ORDER BY count DESC
...
const bestEvent = country => {
  return `SELECT COUNT(*) AS count, event
  FROM GoldMedal
  WHERE country = '${country}'
  GROUP BY event
  ORDER BY count DESC
...
const mostMedaledAthlete = country => {
  return `SELECT COUNT(*) AS count, name 
	FROM GoldMedal
	WHERE country = '${country}'
	GROUP BY name
	ORDER BY count DESC
